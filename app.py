...
st.caption("本部 → 課 → 係 の職務記述書と、グレード別責任範囲を分けて管理する版です。")
...
DEPARTMENT_SECTIONS_MASTER = {
    "人事課": ["採用・解雇", "労務管理", "給与管理"],
    "総務課": ["ライセンス・パーミット", "ドライバー"],
    "情報管理課": ["財務情報", "エリア情報", "車両情報", "災害事故"],
    "経理課": ["DVO係", "GES係", "OLT係", "TAW係"],
    "財務課": ["DVO係", "GES係", "OLT係", "TAW係", "オンラインバンキング係"],
    "エリア運営課": ["カラタオ", "アラベル", "ダバオ"],
    "設備・運搬課": ["在庫管理係", "車両メンテナンス係", "調達・運搬係"],
    "ナーサリー課": ["育苗係", "生産計画係"],
    "マーケティング課": ["折衝係", "契約管理係", "エリアチェック・植付計画係"],
    "プロセス管理課": ["メンテナンスプロセス係", "植付プロセス係"],
    "インサイドコミュニケーション課": ["East", "West"],
    "アウトサイドコミュニケーション課": ["East", "West"],
}
...
GRADE_MASTER_DEFAULT = [
    {
        "grade": "G6",
        "grade_name": GRADE_NAMES["G6"],
        "positioning": "上位者の指示・方針のもとで担当業務を確実に遂行し、組織の一員として基本的な役割と責任を果たすランク。",
        "scope": "個人の担当業務の遂行責任",
        "keywords": "正確性 / 手順遵守 / 誠実さ / 協調性",
        "behavior_points": "担当業務に責任を持って取り組み、マナー・期限・約束を守り、周囲と協力して業務を遂行する。",
    },
    {
        "grade": "G5B",
        "grade_name": GRADE_NAMES["G5B"],
        "positioning": "G6からのステップアップ段階として、一定の自律性をもって業務を遂行するランク。",
        "scope": "担当領域の自律的な遂行責任",
        "keywords": "主体性 / 問題解決 / 柔軟対応 / 協力",
        "behavior_points": "必要な情報を取捨選択し、状況に応じた方法で実行しながら、組織成果に向けて自ら進んで協力する。",
    },
    {
        "grade": "G5A",
        "grade_name": GRADE_NAMES["G5A"],
        "positioning": "G5ランクの上位段階として、求められる職能基準を安定的に満たしているランク。",
        "scope": "複数業務の安定遂行と改善への責任",
        "keywords": "安定性 / 主体性 / 問題解決 / 周囲支援",
        "behavior_points": "G5Bと同じ基準を安定して発揮し、自分の役割を果たしながら周囲支援と改善提案にも取り組む。",
    },
    {
        "grade": "G4",
        "grade_name": GRADE_NAMES["G4"],
        "positioning": "組織運営に関与し、課規模の目標達成および人材・業務の管理責任を担うランク。",
        "scope": "課規模の成果責任 / 人・業務の管理責任",
        "keywords": "計画立案 / 人の管理 / 進捗管理 / 課題解決 / 浸透",
        "behavior_points": "月間目標に必要な施策を立案・展開し、メンバー管理、進捗管理、問題解決、方針浸透を担う。",
    },
    {
        "grade": "G3",
        "grade_name": GRADE_NAMES["G3"],
        "positioning": "組織運営に関与し、事業部規模の目標達成および人材・業務の管理責任を担うランク。",
        "scope": "事業部規模の成果責任 / 組織運営責任",
        "keywords": "年計画 / 部門統括 / 人材管理 / 横断連携 / 意思決定",
        "behavior_points": "年次計画の立案・展開、部門全体の管理、関係部署との調整、部下育成、課題解決を統括する。",
    },
    {
        "grade": "G2",
        "grade_name": GRADE_NAMES["G2"],
        "positioning": "組織運営に関与し、支店規模の目標達成および人材・業務の管理責任を担うランク。",
        "scope": "支店規模の成果責任 / 経営連携責任",
        "keywords": "中期計画 / 経営視点 / 全体最適 / 組織統括 / 横断推進",
        "behavior_points": "中期計画を踏まえた施策決定と展開、組織全体の管理、重要課題解決、経営と現場の接続を担う。",
    },
]
...
# HELPERS
# =========================
def ensure_sections_list(value):
...
def sections_to_pipe(value) -> str:
    return "|".join(ensure_sections_list(value))


def sections_to_bullets(value) -> str:
    sections = ensure_sections_list(value)
    if not sections:
        return "なし"
    return "\n".join([f"・{s}" for s in sections])


def normalize_department_df(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    if "sections" not in out.columns:
        out["sections"] = [[] for _ in range(len(out))]
    out["sections"] = out["sections"].apply(ensure_sections_list)
    return out


def dataframe_to_csv_bytes(df: pd.DataFrame, is_department: bool = False) -> bytes:
    export_df = df.copy()
    if is_department and "sections" in export_df.columns:
        export_df["sections"] = export_df["sections"].apply(sections_to_pipe)
...
def save_snapshot(dept_df: pd.DataFrame, grade_df: pd.DataFrame, memo: str) -> None:
    export_department_df = dept_df.copy()
    export_department_df["sections"] = export_department_df["sections"].apply(ensure_sections_list)
    payload = {
        "department_data": export_department_df.to_dict(orient="records"),
...
def merge_partial_update(base_df: pd.DataFrame, upload_df: pd.DataFrame, key_cols: list[str], update_cols: list[str], is_department: bool = False):
    for col in key_cols:
    ...
    if is_department and "sections" in incoming.columns:
        incoming["sections"] = incoming["sections"].apply(ensure_sections_list)
    ...
    for idx, row in base.iterrows():
        ...
        for col in update_cols:
            ...
            if col == "sections":
                new_val = ensure_sections_list(source.get(col))
                old_val = ensure_sections_list(row.get(col))
    ...
    if is_department:
        base = normalize_department_df(base)
    return base, pd.DataFrame(changes)


def update_master_from_editor(master_df: pd.DataFrame, edited_df: pd.DataFrame, key_cols: list[str], update_cols: list[str], is_department: bool = False) -> pd.DataFrame:
    current = master_df.copy()
    edited = edited_df.copy()
    if is_department and "sections" in edited.columns:
        edited["sections"] = edited["sections"].apply(ensure_sections_list)
    ...
    if is_department:
        current = normalize_department_df(current)
    return current
...
department_df = normalize_department_df(st.session_state.department_df.copy())
...
with c3:
    keyword = st.text_input("キーワード検索", placeholder="説明、係、評価の観点、責任範囲など")
...
if keyword:
    ...
    filtered_department_df = filtered_department_df[
        filtered_department_df.apply(
            lambda row: any(
                kw in str(v).lower() for k, v in row.items() if k != "sections"
            ) or any(kw in s.lower() for s in ensure_sections_list(row["sections"])),
            axis=1,
        )
    ]
...
with tab1:
    ...
    for hq in HQ_STRUCTURE.keys():
        ...
        with st.expander(f"🏢 {hq}", expanded=True):
            for _, row in sub.iterrows():
                ...
                st.write("**係**:")
                st.markdown(f"<div class='card-box'>{sections_to_bullets(row['sections'])}</div>", unsafe_allow_html=True)
    ...
    for hq in HQ_STRUCTURE.keys():
        ...
        section_count = sum(len(ensure_sections_list(v)) for v in sub["sections"])
        summary_rows.append({
            "hq": hq,
            "department_count": len(sub),
...
with tab2:
    ...
    if edit_target == "課マスタ":
        editor_df = filtered_department_df.copy()
        editor_df["sections"] = editor_df["sections"].apply(sections_to_pipe)
...
with tab3:
    ...
    with c1:
        st.download_button(
            "課マスタCSV出力",
            data=dataframe_to_csv_bytes(st.session_state.department_df, is_department=True),
            file_name="department_master_export.csv",
            mime="text/csv",
        )
    with c2:
        st.download_button(
            "課マスタ空テンプレートCSV出力",
            data=minimal_department_template_csv_bytes(),
            file_name="department_master_template.csv",
            mime="text/csv",
        )

    st.caption("係は `|` 区切りで入力してください。例: 採用・解雇|労務管理|給与管理")
    ...
    if uploaded_department_csv is not None:
        try:
            ...
            if "sections" in import_department_df.columns:
                import_department_df["sections"] = import_department_df["sections"].apply(ensure_sections_list)
                preview_df = import_department_df.copy()
                preview_df["sections"] = preview_df["sections"].apply(sections_to_pipe)
            else:
                preview_df = import_department_df.copy()
            ...
            if "hq" in import_department_df.columns and "department" in import_department_df.columns:
                invalid_department_df = validate_import_keys(
                    st.session_state.department_df,
                ...
                if not invalid_department_df.empty:
                    ...
                    if "sections" in invalid_department_df.columns:
                        invalid_department_df["sections"] = invalid_department_df["sections"].apply(sections_to_pipe)
                    st.dataframe(invalid_department_df, use_container_width=True, hide_index=True)

            dept_csv_memo = st.text_input("課マスタCSV更新メモ", placeholder="例: 各課の係を反映", key="dept_csv_memo")
    ...
    with c3:
        st.download_button(
            "グレードマスタCSV出力",
            data=dataframe_to_csv_bytes(st.session_state.grade_df),
            file_name="grade_master_export.csv",
            mime="text/csv",
        )
    with c4:
        st.download_button(
    ...
    if uploaded_grade_csv is not None:
        try:
            ...
            if "grade" in import_grade_df.columns:
                invalid_grade_df = validate_import_keys(
                    st.session_state.grade_df,
                    import_grade_df,
                    key_cols=["grade"],
                )
...
with tab4:
    ...
    if not history:
        ...
        for i, item in enumerate(history):
            ...
            memo = item.get("memo", "")
            hist_department_df = normalize_department_df(pd.DataFrame(item.get("department_data", [])))
            ...
            with st.expander(f"{created_at} | {memo}", expanded=False):
                ...
                if hist_department_df.empty:
                    ...
                    preview_hist_department_df = hist_department_df.copy()
                    preview_hist_department_df["sections"] = preview_hist_department_df["sections"].apply(sections_to_pipe)
                    st.dataframe(preview_hist_department_df, use_container_width=True, hide_index=True)
...
with tab5:
    ...
    st.markdown("### 課マスタCSV例")
    st.code(
        'hq,department,job_title,sections,role_summary,evaluation_points\n'
        '管理本部,人事課,人事課担当,"採用・解雇|労務管理|給与管理",'
        '採用、労務管理、従業員情報管理、教育支援など、人材に関わる業務全般を担う。,'
        '正確性、対応期限遵守、情報管理、関係部署連携、採用・労務運用の安定性',
        language="csv",
    )
...
- 課の下に **複数の係** を持てるようにしています。  
- 係は画面編集では `|` 区切り、保存時はリストとして扱います。  
